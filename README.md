...
file. Running `processnetCDF.py` with the new shp file will process the images and create a new yaml file `chips.yaml` within `outputFolder`. This file will contain the parameters of each chip, which will then be read by the program to crop out the corresponding chips.
